...
export async function POST(req: Request) {
  try {
    ...
    // 1. Create a server-side Supabase client to verify the user session
    ...
    // 2. Verify identity using getUser() - most secure method
    ...
    // 3. Security Check: Ensure the requested userId matches the authenticated user
    if (user.id !== userId) {
    ...
    // 6. Persist Order to Database using Admin client
