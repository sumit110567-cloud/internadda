...
import { createClient } from '@supabase/supabase-js' // Database store karne ke liye
...
export default function ContactPage() {
  ...
  const sendEmail = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Database mein store karein (Table name 'contact_queries' hona chahiye)
      ...
      // 2. EmailJS notification bhejein
      ...
      // 3. Success state update karein
      setIsSubmitted(true)
      
      // 4. 3 second baad automatic refresh/redirect
      setTimeout(() => {
        router.push("/")
      }, 3000)
      ...
      // Error hone par bhi user ko smooth experience dene ke liye alert dikhayein
    ...
    <div className="min-h-screen bg-[#fcfcfd] flex flex-col md:flex-row relative overflow-hidden font-sans">
      ...
      <div className="relative w-full md:w-[35%] p-8 md:p-12 flex flex-col justify-between bg-white border-r border-gray-100 shadow-xl z-10">
          ...
          <div className="space-y-6 pt-4">
            <div className="flex gap-4 items-start group">
              ...
              <div className="space-y-0.5">
                <h4 className="text-[#0A2647] font-bold text-xs tracking-wide uppercase">Verified Support</h4>
                <p className="text-gray-500 text-[10px] leading-relaxed font-medium">Official MSME Registered communication channel.</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="pt-12 mt-8 border-t border-gray-100 flex items-center gap-4">
          ...
          <div className="space-y-0.5">
            <p className="text-[8px] uppercase tracking-[0.4em] text-gray-500 font-black">InternAdda Enterprises</p>
            <p className="text-[7px] text-indigo-600 font-bold font-mono">SECURE_STORE_ENABLED: TRUE</p>
          </div>
        </div>
      </div>
          ...
          {!isSubmitted ? (
            ...
            >
              <div className="mb-8 space-y-1">
                <h2 className="text-2xl font-bold text-[#0A2647] tracking-tight">Submit Query</h2>
                <p className="text-indigo-600 text-xs font-medium uppercase tracking-widest">Data will be stored securely</p>
              </div>
              ...
              <form ref={form} onSubmit={sendEmail} className="space-y-4">
                ...
                <Input name="subject" placeholder="Subject" required className={inputStyles} />
                <Textarea 
                  name="message" 
                  placeholder="Your message..." 
                  required 
                  className="min-h-[120px] bg-white border border-indigo-100 text-[#0A2647] rounded-xl resize-none p-4 text-sm shadow-sm" 
                />
                
                <Button 
                  disabled={isLoading}
                  className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl uppercase text-[10px] font-black tracking-[0.3em] transition-all shadow-lg shadow-indigo-200 mt-2"
                >
                  {isLoading ? <Loader2 className="animate-spin h-4 w-4 mx-auto" /> : "Send Inquiry"}
                </Button>
              </form>
            </motion.div>
          ) : (
            ...
            >
              ...
              <div className="space-y-2">
                <h3 className="text-3xl font-black text-[#0A2647] tracking-tighter uppercase">Query Logged</h3>
                <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest">
                  Success! Your data is stored. Redirecting...
                </p>
              </div>
            </motion.div>
          )}
