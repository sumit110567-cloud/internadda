...
export default function InternshipAssessment() {
  ...
  // --- 1. Gatekeeper Fix (Matching with Webhook & Verify API) ---
  useEffect(() => {
    const verifyAccess = async () => {
      ...
      try {
        // FIX: Hum 'orders' table se PAID status check kar rahe hain latest testId ke liye
        const { data: orders, error } = await supabase
          .from('orders')
          .select('status')
          .eq('user_id', user.id)
          .eq('test_id', String(id)) // Ensure String format
          .eq('status', 'PAID')
          .limit(1)
        ...
        if (orders && orders.length > 0 && !error) {
          ...
          // Double check: Agar webhook delay ho raha hai toh hum 2 second baad re-check kar sakte hain
          setIsAuthorized(false)
        }
      } catch (e) {
      ...
      <header className="fixed top-0 left-0 right-0 h-20 bg-white/80 backdrop-blur-xl border-b border-slate-200 z-40 px-6 md:px-12 flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <span className={cn("text-2xl font-black font-mono tracking-tighter", timeLeft < 300 ? "text-red-500" : "text-[#0A2647]")}>
      ...
      <div className="pt-32 pb-20 px-4 md:px-12 max-w-7xl mx-auto grid lg:grid-cols-[1fr_350px] gap-10">
        <main>
          <motion.div key={currentIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white p-8 md:p-16 rounded-[3.5rem] shadow-sm border border-slate-100 relative overflow-hidden">
            ...
            <h2 className="text-2xl md:text-4xl font-black text-[#0A2647] mb-12 leading-tight tracking-tight">{testData.questions[currentIdx].q}</h2>
            <div className="grid gap-4">
              {testData.questions[currentIdx].options.map((option, i) => (
                <button key={i} disabled={submitting} onClick={() => handleSelectAnswer(i)} className={cn("w-full text-left p-6 md:p-8 rounded-3xl border-2 transition-all font-bold text-lg flex justify-between items-center group", answers[currentIdx] === i ? "border-[#0A2647] bg-blue-50/30 text-[#0A2647]" : "border-slate-100 hover:border-slate-300 text-slate-600 bg-white")}>
                  <span className="flex gap-4"><span className="text-slate-300 font-mono">0{i+1}.</span>{option}</span>
                  <div className={cn("w-6 h-6 rounded-full border-2 transition-all", answers[currentIdx] === i ? "border-[#0A2647] bg-[#0A2647] shadow-[0_0_0_4px_rgba(10,38,71,0.1)]" : "border-slate-200")} />
                </button>
              ))}
            </div>
            <div className="mt-16 flex justify-between items-center">
               <Button variant="ghost" disabled={currentIdx === 0} onClick={() => setCurrentIdx(prev => prev - 1)} className="rounded-2xl px-8 h-14 font-black text-slate-400 hover:text-[#0A2647]">Previous</Button>
               {currentIdx === testData.questions.length - 1 ? (
                 <Button onClick={() => finishTest()} className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl px-12 h-16 font-black text-lg shadow-xl shadow-emerald-500/20">Submit Final Test</Button>
               ) : (
                 <Button onClick={() => setCurrentIdx(prev => prev + 1)} className="bg-[#0A2647] hover:bg-blue-900 text-white rounded-2xl px-12 h-16 font-black text-lg shadow-xl shadow-blue-900/20">Next Question <ChevronRight className="ml-2" /></Button>
               )}
            </div>
          </motion.div>
        </main>
        <aside className={cn("fixed lg:relative inset-y-0 right-0 w-80 bg-white border-l lg:border-none border-slate-200 z-50 p-8 transform transition-transform lg:transform-none shadow-2xl lg:shadow-none", showSidebar ? "translate-x-0" : "translate-x-full lg:translate-x-0")}>
          <div className="lg:sticky lg:top-32">
            ...
            <div className="grid grid-cols-4 gap-3">
              {testData.questions.map((_, i) => (
                <button key={i} onClick={() => { setCurrentIdx(i); if(window.innerWidth < 1024) setShowSidebar(false) }} className={cn("h-12 rounded-xl flex items-center justify-center font-black text-sm transition-all border-2", currentIdx === i ? "border-[#0A2647] bg-[#0A2647] text-white" : answers[i] !== undefined ? "border-blue-100 bg-blue-50 text-blue-600" : "border-slate-100 text-slate-400 hover:border-slate-200")}>
                  {i + 1}
                </button>
              ))}
            </div>
