...
function decodePayload(raw: string): TestMeta | null {
  try {
    const base64 = raw.replace(/-/g, '+').replace(/_/g, '/')
    const padded  = base64 + '=='.slice((base64.length * 3) % 4 ? (base64.length * 3) % 4 - 1 : 0)
    const json    = decodeURIComponent(atob(padded))
    const p       = JSON.parse(json)
    if (!p.n || !p.s || !p.d) return null
    return { name: p.n, university: p.u || '', sector: p.s, duration: p.d, code: p.c || '' }
  } catch { return null }
}
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10">
        ...
        >
          {/* Header */}
          <div className="bg-[#1a1063] rounded-t-2xl px-6 py-6 text-center relative overflow-hidden">
            ...
            <div className="relative">
              <div className="w-14 h-14 rounded-xl mx-auto mb-3 flex items-center justify-center text-white font-black text-xl"
                style={{ background: 'rgba(255,255,255,0.14)', border: '1.5px solid rgba(255,255,255,0.2)' }}>
                {meta.name[0].toUpperCase()}
              </div>
              <h2 className="text-white font-extrabold text-[1.1rem] tracking-tight">{meta.name}</h2>
              {meta.university && <p className="text-indigo-300 text-[12px] mt-0.5 font-medium">{meta.university}</p>}
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 border-t-0 rounded-b-2xl p-6">
            ...
            <div className="grid grid-cols-3 gap-2.5 mb-4">
              {[
                ...
                <div key={s.l} className="bg-slate-50 border border-slate-100 rounded-xl p-3 text-center">
                  ...
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide mt-1">{s.l}</p>
                </div>
              ))}
            </div>
            ...
            <ul className="space-y-1.5 mb-6">
              {[
                ...
                `Timer starts when you click Begin — ${meta.duration} minutes total.`,
                'Jump between questions freely using the dots.',
                'Marksheet available right after submission.',
              ].map((r, i) => (
...
function ActiveTest({
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) { clearInterval(intervalRef.current); submit(); return 0 }
        return prev - 1
      })
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b border-slate-100 sticky top-0 z-50">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md bg-[#1a1063] flex items-center justify-center text-white text-[10px] font-black">I</div>
            <span className="text-[12px] text-slate-500 font-semibold truncate max-w-[120px] sm:max-w-none">{meta.sector}</span>
          </div>
          <div className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-white transition-colors ${warn ? 'bg-rose-500' : 'bg-slate-900'}`}>
            <Clock size={12} />
            <span className="font-mono font-bold text-[13px] tabular-nums">{mins}:{secs.toString().padStart(2,'0')}</span>
          </div>
        </div>
        {/* Progress line */}
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-6 pb-16">
                ...
                return (
                  <button key={i} onClick={() => select(i)}
                    className={`w-full flex items-center gap-3 p-4 rounded-xl border-2 text-left transition-all ${
                      sel ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 bg-white hover:border-indigo-200 hover:bg-slate-50'
                    }`}>
                    ...
                    <span className={`text-[13.5px] font-medium leading-snug flex-1 ${sel ? 'text-indigo-800 font-semibold' : 'text-slate-700'}`}>
                      {opt}
                    </span>
        ...
        {currentQ === 9 && answers.some(a => a === null) && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <AlertCircle size={12} />
            {answers.filter(a => a === null).length} question{answers.filter(a => a === null).length > 1 ? 's' : ''} unanswered — you can still submit.
          </motion.p>
        )}
      </div>
    </div>
...
function Result({ meta, questions, answers, timeUsed }: {
  meta: TestMeta; questions: Question[]; answers: (number | null)[]; timeUsed: number
}) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 pb-16">
        ...
        >
          ...
          <div aria-hidden className="absolute inset-y-0 right-0 w-2/5"
            style={{ background: 'linear-gradient(to left, rgba(99,102,241,0.22), transparent)' }} />

          <div className="relative">
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] mb-1.5" style={{ color: 'rgba(196,181,253,0.8)' }}>
              Skill Assessment · {meta.sector}
            </p>
            <p className="text-white/50 text-[12.5px] font-semibold mb-3">{meta.name} · {meta.university}</p>

            <div className="font-black leading-none mb-3 text-white" style={{ fontSize: 'clamp(3rem,12vw,5rem)' }}>
              {score}<span className="text-white/35" style={{ fontSize: '.38em' }}>/40</span>
            </div>

            {/* Grade */}
            <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 mb-5"
              style={{ background: `${gi.color}28`, border: `1px solid ${gi.color}55` }}>
              <span className="text-white font-bold text-[13px]">Grade {gi.grade}</span>
              <span className="text-white/60 text-[12px]">{gi.label}</span>
            </div>

            {/* Stats chips */}
            <div className="flex flex-wrap justify-center gap-2">
              {[
                { l: `✓ ${correct} Correct`,  c: 'rgba(5,150,105,0.2)',  b: 'rgba(5,150,105,0.35)',  t: '#6ee7b7' },
                { l: `✗ ${wrong} Wrong`,       c: 'rgba(225,29,72,0.2)', b: 'rgba(225,29,72,0.35)',  t: '#fca5a5' },
                { l: `⊘ ${skipped} Skipped`,  c: 'rgba(217,119,6,0.18)',b: 'rgba(217,119,6,0.35)',  t: '#fcd34d' },
                { l: `⏱ ${mUsed}m ${sUsed}s`, c: 'rgba(255,255,255,0.07)',b:'rgba(255,255,255,0.14)', t:'rgba(255,255,255,0.65)'},
              ].map(c => (
        ...
        {/* Internship nudge — clean, not cringe */}
        <div className="bg-[#1a1063] rounded-2xl px-6 py-7 text-center relative overflow-hidden no-print">
          <div aria-hidden className="absolute inset-0 opacity-[0.04]">
            <svg width="100%" height="100%"><defs><pattern id="cp" width="22" height="22" patternUnits="userSpaceOnUse">
              <circle cx="1" cy="1" r="0.8" fill="white" /></pattern></defs>
              <rect width="100%" height="100%" fill="url(#cp)" /></svg>
          </div>
          <div className="relative">
            <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(196,181,253,0.75)' }}>Next Step</p>
            <h3 className="text-white font-extrabold text-[1.15rem] tracking-tight mb-1.5">
              Browse open internships on Internadda.
            </h3>
            <p className="text-[13px] mb-5" style={{ color: 'rgba(199,210,254,0.65)' }}>
              500+ verified companies · no login needed to browse
            </p>
            <Link href="/internships"
      ...
      <style>{`@media print { .no-print{display:none!important} nav{display:none!important} body{background:#fff!important} }`}</style>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-20">
        ...
        <h2 className="text-[1.2rem] font-extrabold text-slate-900 mb-2">Invalid test link</h2>
        <p className="text-slate-500 text-[13px] max-w-xs mb-6">
          This link may be corrupted or expired. Ask your admin to regenerate the link.
        </p>
...
export default function TestPage() {
  ...
  useEffect(() => {
    ...
    if (!decoded) { setPhase('notfound'); return }
    setMeta(decoded)
    setPhase('lobby')
  }, [params])
  ...
  const handleStart = () => {
    if (!meta) return
    // getQuestions accepts any string sector, cast safely
  ...
  if (phase === 'result' && meta)              return <Result meta={meta} questions={questions} answers={finalAnswers} timeUsed={finalTime} />
  return null
}
